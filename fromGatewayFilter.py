...
def fromGatewayFilter(message):
	...
	if 'context' in message:
		if 'cgwSessionID' in message['context']:
			logging.info("API Call -> SessionID: " + message['context']['cgwSessionID'])
		else:
			logging.info('API Call -> Serious Error, no cgwSessionID in Context')
...
def setBargeIn(message):
	if 'context' in message:
		if voiceProxySettings.BARGE_IN_ENABLED:
			message['context']['cgwAllowBargeIn'] = 'Yes'
		else:		
			message['context']['cgwAllowBargeIn'] = 'No'
